...
if (!isset($_SESSION['user_email'])) {
    // Redirect to feed page if session email couldn`t save
...
// Fetch user's name using the session user_user
...
$firstName = "Default Name"; // Fallback fname
$lastName = "Default Name2"; // Fallback fname
...
$stmt = $conn->prepare("SELECT links FROM users WHERE email = ?");
$stmt->bind_param("s", $user_email);
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save_profile'])) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                if ($fileError === 0) {
                    if ($fileSize < 5 * 1024 * 1024) {
                        ...
                        if (move_uploaded_file($fileTempName, $uploadPath)) {
                            $sql = 'UPDATE users SET username = ?, email = ?, bio = ?, gender =?, relationship = ?, profile_pic = ? WHERE $user_email = ?';
                            $stmt = $conn->prepare($sql);
                            $stmt->bind_param('sssssss', $username, $email, $bio, $gender, $relationship, $newFileName, $user_email);
            ...
            if ($stmt->execute()) {
                echo "Data was successfully Modified";
            } else {
    ...
    if (isset($_POST['save_links'])) {
        ...
        $stmt->bind_param('si', $linkjson, $user_email);
        $stmt->execute();
        $stmt->close();
     ...
     //Hapa tunaandika code za kubadilisha password
    if (isset($_POST['change_password'])) {
        ...
        if ($new_password !== $repeat_password) {
            ...
            $sql = "SELECT email, pass_word FROM users WHERE email = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("s", $user_email);
            $stmt->execute();
            $stmt->store_result();
            ...
            if ($stmt->num_rows > 0) {
                ...
                if ($current_password == $stored_hash) {
                    $stmt = $conn->prepare('UPDATE users SET pass_word = ?
                     WHERE email = ?');
                            ...
                            <div  class="hidden bg-white rounded-lg drop-shadow-xl dark:bg-slate-700 w-64 border2"
                                ...
                                <a href="timeline.html">
                                    <div class="p-4 py-5 flex items-center gap-4">
                                        ...
                                        <div class="flex-1">
                                            <h4 class="text-sm font-medium text-black"><?php echo($firstName . " ". $lastName);?></h4>
                                            <div class="text-sm mt-1 text-blue-600 font-light dark:text-white/70"><?php echo $user_email;?></div>
                                        </div>
                                    </div>
                                </a>
            ...
            <div class="max-w-3xl mx-auto">
                ...
                <div class="box relative rounded-lg shadow-md">
                    ...
                    <div class="flex md:gap-8 gap-4 items-center md:p-8 p-6 md:pb-4">
                        ...
                        <div class="flex-1">
                            <h3 class="md:text-xl text-base font-semibold text-black dark:text-white"><?php echo($firstName . " ". $lastName);?></h3>
                            <p class="text-sm text-blue-600 mt-1 font-normal"><?php echo $user_email;?></p>
                        </div>
                    </div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="md:flex items-center gap-10">
                                    ...
                                    <div class="flex-1 max-md:mt-4">
                                        <input type="text" name="username"
                                            value=""
                                            placeholder="<?php echo htmlspecialchars($user['username']); ?>" class="lg:w-1/2 w-full">
                                    </div>
                                </div>
                                ...
                                <div class="md:flex items-start gap-10 ">
                                    ...
                                    <div class="flex-1 flex items-center gap-5 max-md:mt-4">
                                        ...
                                        <button
                                            type="button"
                                            class="change-button px-4 py-1 rounded-full bg-slate-100/60 border dark:bg-slate-700 dark:border-slate-600  dark:text-white"
